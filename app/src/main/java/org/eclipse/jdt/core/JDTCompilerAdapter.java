 ...
 * </p>
 * <p>
 * For more information on Ant check out the website at <a href="http://jakarta.apache.org/ant/">...</a> .
 * </p>
 *
 * @since 2.0
 */
...
public class JDTCompilerAdapter extends DefaultCompilerAdapter {
	...
	public boolean execute() throws BuildException {
		...
		try {
			//$NON-NLS-1$
			String compilerClass = "org.eclipse.jdt.internal.compiler.batch.Main";
			...
			Method compile = c.getMethod("compile", String[].class); //$NON-NLS-1$
	...
	protected Commandline setupJavacCommand() throws BuildException {
		...
		var classpath = new org.apache.tools.ant.types.Path(this.project);
		/*
		...
		org.apache.tools.ant.types.Path compileSourcePath = null;
		if (getSourcepathMethod != null) {
			try {
				compileSourcePath = (org.apache.tools.ant.types.Path) getSourcepathMethod.invoke(this.attributes, (Object[]) null);
		...
		/*
		 * release option.
		 * Note from MrBoomDev: I don't know if this line is important
		 * but i have commented it because there's no such variable.
		 */
		...
		if (compilerArgs != null) {
			...
			if (length != 0) {
				for (int i = 0; i < length; i++) {
					...
					if (this.logFileName == null && "-log".equals(arg) && ((i + 1) < length)) { //$NON-NLS-1$
	 ...
	 */
	private void createClasspathArgument(Commandline cmd, org.apache.tools.ant.types.Path classpath) {
	...
	protected void logAndAddFilesToCompile(Commandline cmd) {
		...
		for (int i = 0; i < this.compileList.length; i++) {
			...
			cmd.createArgument().setValue(arg);
			niceSourceList.append("    ").append(arg).append(System.lineSeparator()); //$NON-NLS-1$
		}

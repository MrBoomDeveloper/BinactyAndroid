...
import com.mrboomdev.java.tools.DiagnosticListener;
...
public class EclipseCompilerImpl extends Main {
	...
	protected void handleLocations() {
		...
		Iterable<? extends File> locationFiles;
	...
	protected List<Classpath> getPlatformLocations(ArrayList<FileSystem.Classpath> fileSystemClasspaths, File file) {
		return Util.collectPlatformLibraries(file);
	}
	@Override
				...
				Diagnostic<JavaFileObject> diagnostic = new Diagnostic<>() {
					...
					public Kind getKind() {
						...
						if(problem.isWarning()) {
							return Kind.WARNING;
						} else if(problem instanceof DefaultProblem && problem.isInfo()) {
							return Kind.NOTE;
						}
						return Kind.OTHER;
					}
					...
					public JavaFileObject getSource() {
						if(problem instanceof DefaultProblem) {
							char[] originatingName = problem.getOriginatingFileName();
	...
	static class Jsr199ProblemWrapper extends DefaultProblem {
